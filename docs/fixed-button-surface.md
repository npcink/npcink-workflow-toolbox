...
| Media Library Optimization | Admin Image Handling | administrator-selected time and image-type range frozen as an exact SHA-256 manifest | `toolbox_media_optimization_batch.v1`, Cloud `media_derivative_result.v3`, and Toolkit replacement/restore history | foreground Toolbox loop, Toolkit media abilities, and Cloud Addon/Cloud derivative runtime | one exact-manifest administrator confirmation delegates each qualified replacement or restore to Toolkit | `default_button_v1`; bounded `strong_local_confirmation` exception from ADR-015, no Core proposal, background queue, Cron execution, automatic backup deletion, or Cloud WordPress write |
