...
| Related Article Candidates | Editor Content Support | current draft context plus Cloud Site Knowledge vector evidence | `related_article_candidates.v1` | Cloud-managed Site Knowledge semantic retrieval through Cloud Addon | manual editor reading, opening, copying, or citation | `default_button_v1`; no rule fallback presented as AI, no body insertion, no frontend related-articles block, no WordPress write |
