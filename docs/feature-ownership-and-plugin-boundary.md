...
| `strong_local_confirmation` | Narrow present-user direct apply with exact preview or manifest, native capabilities, and compensation | Toolbox only when an accepted ADR defines the exact action; ADR-015 covers the sole current exact media manifest lane, while ADR-017 retires the former single-image lane |
