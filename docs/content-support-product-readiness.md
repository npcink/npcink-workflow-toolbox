...
| Internal-link candidates | Editor Content Support exposes `internal_links` over bounded article context, optional Cloud Site Knowledge evidence, and a bounded local Toolkit graph scan, then delegates candidate assembly to `npcink-abilities-toolkit/resolve-internal-link-targets`. | Accepted for current phase. Exact phrase matches can be applied, undone, copied, opened, or ignored. Candidate order may receive a bounded orphan/shared-topic boost; duplicate targets, duplicate placements, existing links, excessive article density, and crowded paragraphs are rejected. | No automatic insertion, backend post-content patch, hidden save, or link graph control plane. Persistence occurs only through the author's normal Update or Publish action. |
| Image candidates and media optimization | Editor Content Support exposes `image_candidates`; Toolbox owns image-source UX and delegates review artifact projection to `npcink-abilities-toolkit/build-image-candidate-review-artifact`. Toolbox admin owns the ADR-015 Media Library Optimization surface over an exact SHA-256 manifest. | Accepted for current phase. The default batch checks representative Cloud-qualified results, records one present-administrator confirmation, and resumes a bounded foreground run. Advanced single-image controls remain separate. | Image-source candidates remain candidates. For default Media Library Optimization, Toolkit alone performs each replacement, backup, lineage record, and restore; external Agent and open-ended paths remain Core-governed. |
