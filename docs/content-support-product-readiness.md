...
| Image candidates and media optimization | Editor Content Support exposes `image_candidates`; Toolbox owns image-source UX and delegates review artifact projection to `npcink-abilities-toolkit/build-image-candidate-review-artifact`. Toolbox admin owns the ADR-015 Media Library Optimization surface over an exact SHA-256 manifest. | Accepted for current phase. The default batch checks representative Cloud-qualified results, records one present-administrator confirmation, and resumes a bounded foreground run. | Image-source candidates remain candidates. Single-image import, replacement, and restore use Adapter/Core/Toolkit. For default Media Library Optimization, Toolkit alone performs each replacement, backup, lineage record, and restore. |
